...
__device__ void getEigenvalues( float * eigenVals, float d_t1, float d_t2, float d_t3){
...
__global__ void getFeatures(float *d_imgIn, float *d_t1, float *d_t2,
                            float *d_t3, float *d_imgOut, int nc, int w, int h,
                            float alpha, float beta) {

  // getFeatures() finds features in an image; marking corners red and edges
  // yellow
  ...
  for ( int c = 0; c < nc; c ++){
	 d_imgOut[ind + w * h * c] = d_imgIn[ind + w * h * c];
  }

  float Vals[2] = {0, 0};
  float *eigenVals = Vals;
  
  getEigenvalues(  eigenVals, d_t1[ind], d_t2[ind], d_t2[ind]);

  if (eigenVals[0] >= alpha) {
	// mark image red
	d_imgOut[ind] = 1;
	d_imgOut[ind + w * h] = 0;
	d_imgOut[ind + 2 * w * h] = 0;
  }
  // this assumes alpha > beta > 0
  else if (eigenVals[0] <= beta && eigenVals[1] >= alpha) {
	...
	// darken image everwhere else
	for (int c = 0; c < nc; c++)
	  d_imgOut[ind + w * h * c] *= 0.5;
  }
}
...
__global__ void computeM(float *d_gradStencilX, float *d_gradStencilY,
  ...
  if (x < w && y < h) {
    ...
    for (int i = 0; i < nc; i++) {
      d_m1[ind] +=
          d_gradStencilX[ind + w * h * i] * d_gradStencilX[ind + w * h * i] * 10;
      d_m2[ind] +=
          d_gradStencilX[ind + w * h * i] * d_gradStencilY[ind + w * h * i] * 10;
      d_m3[ind] +=
          d_gradStencilY[ind + w * h * i] * d_gradStencilY[ind + w * h * i] * 10;
    }
  }
}
...
// For camera mode: Make a loop to read in camera frames
  ...
  while (cv::waitKey(30) < 0) {
	...
	// for X
	globalConvolution<<<grid, block>>>(d_imgOut, d_gradStencilX, d_gradX,
										   nc, w, h, 3, 3);
	// for Y
	...
	globalConvolution<<<grid, block>>>(d_m1, d_kernel, d_t1, 1, w, h, w_k,
									   h_k);

	globalConvolution<<<grid, block>>>(d_m2, d_kernel, d_t2, 1, w, h, w_k,
									   h_k);

	globalConvolution<<<grid, block>>>(d_m3, d_kernel, d_t3, 1, w, h, w_k,
									   h_k);
	getFeatures<<<grid, block>>>(d_imgIn, d_t1, d_t2, d_t3, d_imgOut, nc, w, h, alpha, beta);
