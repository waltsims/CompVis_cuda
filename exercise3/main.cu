...
__global__ void gpuGammaCorrect( float *imgIn, float *imgOut, int nc, int w, int h, float gamma){
	int x = threadIdx.x + blockDim.x * blockIdx.x;
	int y = threadIdx.y + blockDim.y * blockIdx.y;
	int ind = x + w*y;
	if (x < w && y < h ) {
		for ( int i = 0; i < nc ; i++){
			imgOut[ind + i * h * w] = pow(imgIn[ind + i * h * w], gamma);
		}
	};
}

__host__ void cpuGammaCorrect( float *imgIn, float *imgOut, int nc, int w, int h, float gamma){
	int i; int k; int j;
	for ( i = 0 ; i < nc ; i++){
		for( j = 0 ; j < w ; j++){
			for( k = 0; k < h; k++){
				imgOut[i * h * w + k + j * h] = pow(imgIn[i * h * w + k + j * h], gamma);
			}
		}
...
int main(int argc, char **argv)
{
	...
	// CPU implimentation
	//cpuGammaCorrect(imgIn, imgOut, nc,	w, h, gamma);

	//GPU implimentation
	float *d_imgIn; float *d_imgOut;
	...
	gpuGammaCorrect <<< grid, block >>> (d_imgIn, d_imgOut, nc, w, h, gamma);
